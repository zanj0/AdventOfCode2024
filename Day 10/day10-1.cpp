...
void Out(vector<pair<lli, lli>>& v) {
	for (auto& it : v) {
		cout << it.ff << " " << it.ss << endl;
	}
	cout << endl;
}
set<pair<lli, lli>> done;

lli Dfs(int i, int j, int required) {
	if (!IsInside(i, j) || (v[i][j] - '0') != required) return 0;
	if (v[i][j] == '9') {
		done.insert({i, j});
		return 1;
	}
	lli ret = 0;
	for (int k = 0 ; k < 4; k++) {
		ret += Dfs(i + dir[k][0], j + dir[k][1], required + 1);
	}
	return ret;
}
